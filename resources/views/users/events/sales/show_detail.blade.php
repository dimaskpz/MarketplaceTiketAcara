...
@section('content')
  ...
  <div class="w3-container w3-padding-32">
    ...
    <p>
      <form  action="{{ route('Event.Komisi.Show', ['user_id' => $transaksi->User->id]) }}" method="post">
        <input type="hidden" name="acara_id" value="{{$transaksi->acara_id}}">
        <input type="hidden" name="user_id" value="{{$transaksi->User->id}}">
        <button type="submit" name="submit" class="w3-btn w3-black">Kembali</button>
        {{ csrf_field() }}
      </form>
